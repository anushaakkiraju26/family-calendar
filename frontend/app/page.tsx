...
const initialActivities:Activity[] = [
  ...
  {id:'db-13',title:'Parent-teacher conference',date:'2026-09-17',time:'14:30',endTime:'15:15',person:'Leo',parent:'Anusha',location:'Reed Elementary',color:colors[1],note:'Overlaps normal school hours.'},
...
const schoolActivities:Activity[] = [
  {id:'s1',title:'Spirit Day: Blue & Gold',date:'2026-08-28',time:'09:00',endTime:'15:00',person:'School',parent:'Reed Elementary',location:'Reed Elementary',color:'#8a769d',source:'school',note:'Wear blue & gold or a Reed shirt.'},
  {id:'s2',title:'Back to School Night',date:'2026-09-01',time:'16:30',endTime:'18:00',person:'School',parent:'Reed Elementary',location:'Reed Elementary',color:'#8a769d',source:'school',note:'Timed school event; check family transportation and supervision.'},
  {id:'s3',title:'Picture Day',date:'2026-09-02',time:'09:00',endTime:'15:00',person:'School',parent:'Reed Elementary',location:'Reed Elementary',color:'#8a769d',source:'school',note:'No exact time was published.'},
  {id:'s4',title:'Activity Day: Blacktop Games',date:'2026-09-04',time:'09:00',endTime:'15:00',person:'School',parent:'Reed Elementary',location:'Reed Elementary',color:'#8a769d',source:'school',note:'No exact time was published.'},
  {id:'s5',title:'No School: Labor Day',date:'2026-09-07',time:'09:00',endTime:'15:00',person:'School',parent:'Reed Elementary',location:'School closed',color:'#b46755',source:'school',note:'School is closed; normal school-hour assumptions do not apply.'},
  {id:'s6',title:'Bingo Night',date:'2026-09-18',time:'18:00',endTime:'19:30',person:'School',parent:'Reed Elementary',location:'Reed Elementary',color:'#8a769d',source:'school',note:'Timed school event.'},
];
...
function EventDetails({item,onClose,onAddGuest,onAddReminder,onRemoveGuest}:{item:Activity;onClose:()=>void;onAddGuest:(e:FormEvent<HTMLFormElement>)=>void;onAddReminder:(e:FormEvent<HTMLFormElement>)=>void;onRemoveGuest:(id:string)=>void}){
  ...
  return <section className="event-modal" role="dialog" aria-modal="true" aria-labelledby="event-title">
    ...
    {item.source==='school'?<div className="school-source"><strong>Supplied Reed Elementary 2026–2027 calendar</strong><p>This is reviewed reference data, not a live school feed. Dates are subject to change.</p></div>:<><div className="guest-section"><div className="modal-heading"><div><p className="eyebrow">GUESTS</p><h3>Family & friends</h3></div><span>{guests.length}</span></div>
...
function WeeklyReview({days,activities}:{days:{date:string;label:string;day:number}[];activities:Activity[]}){
  ...
  const findings=inWeek.flatMap(item=>{
    ...
    if(item.source==='school')notes.push(`${item.title}: Reed calendar planning note`);
